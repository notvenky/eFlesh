...
parser.add_argument('input_band', help='input band mesh')
...
print("Starting step 1: Parsing band mesh file...")
mesh_band = meshio.read(args.input_band)
band_vertices = mesh_band.points
band_elements = mesh_band.cells[0][1]

components = connectivity_tools.compute_connected_components(band_vertices, band_elements)
band_boundary = connectivity_tools.find_component_boundary(components[0], band_vertices, band_elements)
band_holes = connectivity_tools.find_component_holes(components[0], band_vertices, band_elements)

band_polygon = Polygon(band_boundary, holes=band_holes)
print(band_polygon)
...
print("Step 3: Identifying minimum densities for each cut cell, according to remaining area after removing band...")
...
for k, ce in enumerate(cut_cells):
    ...
    remaining_poly = polygon.difference(band_polygon)
