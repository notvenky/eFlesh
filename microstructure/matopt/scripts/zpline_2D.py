...
def _bspev_and_cgrid(x, tables, coef_u=poly_coefs[0], coef_v=poly_coefs[0]):
    bu, iu = _bspev_and_c(x[0], tables[0], coef_u)
    bv, iv = _bspev_and_c(x[1], tables[1], coef_v)

    #print(np.outer(bu, bv))

    return np.outer(bu, bv), (iu.reshape(-1, 1), iv.reshape(1, -1))


def test_2d():
    c = np.random.random((13, 13))
    # c = np.ones((13,13))
    # c = np.eye(13)
    tck = [
        np.asarray([0, 0, 0] + list(range(10)) + [10, 10, 10, 10]),
        np.asarray([0, 0, 0] + list(range(10)) + [10, 10, 10, 10]) * 2,
        c.ravel(order='C'), 3, 3
    ]

    bs = bisplev(10 - 1.5, 10 - 1.5, tck)
    print(bs)

    cbs = CubicBiSpline(start=[0, 0], resolution=[0.1, 0.2], width=[10, 20], coef=[c])
    y = cbs.ev([1 - 0.15, 1 - 0.15])
    print(y - bs)
    assert abs(y - bs) < 1e-9
    print('yes')
    ...
    cbs = CubicBiSpline(start=[-6.0, -6.0], resolution=[0.2, 0.2], width=[60, 60])

    cbs.interpolate(x, np.array(y))

    x1_sampling = np.arange(-6, 6.1, 0.5)
    x2_sampling = np.arange(-6, 6.1, 0.5)
    Zbs = np.zeros((len(x1_sampling), len(x2_sampling)))
    X1_sampling, X2_sampling = np.meshgrid(x1_sampling, x2_sampling)

    for i in range(len(x1_sampling)):
        for j in range(len(x2_sampling)):
                x1_ij = X1_sampling[i, j]
                x2_ij = X2_sampling[i, j]
                Zbs[i, j] = cbs.ev([x1_ij, x2_ij])

    # Error:
    total_error = 0.0
    for i in range(len(x1)):
        for j in range(len(x2)):
                error = abs(Zbs[i, j] - Y[i, j])
...
class CubicBiSpline:
    ...
    def __call__(self, x):
        x = self.transform(x)
        outer, ixgrid = _bspev_and_cgrid(x, self.table)
    ...
    def interpolate(self, X, f, regularization_points = None):
        ...
        self.A = scipy.sparse.csr_matrix((self.RCD[:, 2], (self.RCD[:, 0].astype(np.int32), self.RCD[:, 1].astype(np.int32))),
                                    shape=(X.shape[0] + num_laps_rows, (len(self.table[0]) + 2) * (len(self.table[1]) + 2)))
        ...
        self.coef = coef.reshape(len(self.table[0]) + 2, len(self.table[1]) + 2)
    ...
    def _global_basis_row(cls, x, tables):
        outer, ixgrid = _bspev_and_cgrid(x, tables)
        index = np.ravel_multi_index(ixgrid, (len(tables[0]) + 2, len(tables[1]) + 2))
        # return cols and data
        return list(zip(index.ravel(), outer.ravel()))

    @classmethod
    def _global_basis_d2_row(cls, x, tables):
        dim = (len(tables[0]) + 2, len(tables[1]) + 2)

        outer, ixgrid = _bspev_and_cgrid(x, tables, coef_u=poly_coefs[0], coef_v=poly_coefs[2])
        ind = np.ravel_multi_index(ixgrid, dim).ravel()
        out = outer.ravel()
        row1 = list(zip(ind, out))

        outer, ixgrid = _bspev_and_cgrid(x, tables, coef_u=poly_coefs[1], coef_v=poly_coefs[1])
        ind = np.ravel_multi_index(ixgrid, dim).ravel()
        out = outer.ravel()
        row2 = list(zip(ind, math.sqrt(2) * out))

        outer, ixgrid = _bspev_and_cgrid(x, tables, coef_u=poly_coefs[2], coef_v=poly_coefs[0])
        ind = np.ravel_multi_index(ixgrid, dim).ravel()
        out = outer.ravel()
        row3 = list(zip(ind, out))
